...
class AOTInterceptionPjrtClient : public PjRtClient {
 public:
  ...
  static Platform PlatformFromName(absl::string_view platform_name);

  // Returns the on-disk executables subdirectory segment ("gpu" or "cpu") for a
  // Platform.
  static absl::string_view PlatformSubdir(Platform platform);

  // Exposed static helpers for tests and the golden artifact generator.
