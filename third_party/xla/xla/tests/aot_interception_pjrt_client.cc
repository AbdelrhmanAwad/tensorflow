...
AOTInterceptionPjrtClient::DeserializeToHumanReadable(
    absl::string_view serialized, Platform platform) {
  HumanReadableAotExecutable unpacked;

  if (platform == Platform::kGpu) {
...
Platform AOTInterceptionPjrtClient::PlatformFromName(
    absl::string_view platform_name) {
  if (platform_name == "gpu" || platform_name == "cuda" ||
      platform_name == "rocm") {
    return Platform::kGpu;
  }
  return Platform::kCpu;
}

absl::string_view AOTInterceptionPjrtClient::PlatformSubdir(Platform platform) {
  return platform == Platform::kGpu ? "gpu" : "cpu";
}
...
absl::Status AOTInterceptionPjrtClient::VerifyAgainstGolden(
  ...
  const Platform platform = PlatformFromName(inner_client_->platform_name());
...
AOTInterceptionPjrtClient::PackArtifactForInnerClient() {
  ...
  const Platform platform = unpacked.backend_executable_case() ==
